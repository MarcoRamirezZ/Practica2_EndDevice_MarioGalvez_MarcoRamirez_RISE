...
PUBLIC void vAppInitOTA(void)
{
    ...
    sNvmDefs.u8FlashDeviceType = 2; // E_FL_CHIP_AUTO works in 69
#endif
    ...
    else
    {
        ...
        DBG_vPrintf(TRACE_APP_OTA, "Valid %d Nwk Addr %04x Ieee %016llx\n",
                sNodeState.bValid, sNodeState.u16NwkAddrOfServer, sNodeState.u64IeeeAddrOfServer);
        if (sNodeState.bValid == TRUE)
        {
...
PUBLIC void vRunAppOTAStateMachine(void)
{
    ...
    if( E_RUNNING ==  sNodeState.eNodeState)
    {
...
PUBLIC void vHandleAppOtaClient(tsOTA_CallBackMessage *psCallBackMessage)
{
 ...
 //       vStartEffect(E_CLD_IDENTIFY_EFFECT_OKAY);
    ...
    if(psCallBackMessage->eEventId ==  E_CLD_OTA_BLOCK_RESPONSE_TAG_OTHER_THAN_UPGRADE_IMAGE_BITMAP_CRC)
    {
        DBG_vPrintf(TRACE_APP_OTA|OTA_LNT,"E_CLD_OTA_BLOCK_RESPONSE_TAG_OTHER_THAN_UPGRADE_IMAGE_BITMAP_CRC\n\n");
    }
...
PUBLIC void vDumpFlashData(uint32 u32FlashByteLocation, uint32 u32EndLocation)
{
    ...
    for (;u32FlashByteLocation<=u32EndLocation;u32FlashByteLocation +=16 )
    {
        ...
        EEPROM_ReadData(16, u32FlashByteLocation, au8Data);
        uint8 u8Len;
...
#endif /* #ifndef CPU_MKW41Z512VHT4 */
}
...
PRIVATE uint8 u8VerifyLinkKey(tsOTA_CallBackMessage *psCallBackMessage)
{
    ...
    if((
       ...
       ) && false) /* Workaround - TO BE MODIFIED */
    {
        bKeyVerified = TRUE;
        ...
        EEPROM_ReadData(0x10, u32LnkKeyLocation, au8DownloadedLnkKey);
        ...
        if(bEncExternalFlash)
        {
            ...
            //bAHI_FullFlashRead(u32IVLocation,0x10,au8Iv);
             EEPROM_ReadData(0x10, u32IVLocation, au8Iv);
...
PUBLIC void vHandleMatchDescriptor( ZPS_tsAfEvent  * psStackEvent)
{
    ...
    if (sMatchDescRsp.u8Status == 0)
    {
        ...
        if(sMatchDescRsp.u8MatchLength != 0)
        {
            ...
            sNodeState.u8OTAserverEP = ((pdum_tsAPduInstance*)hAPduInst)->au8Storage[u32Location];
            sNodeState.u16NwkAddrOfServer = sMatchDescRsp.u16NwkAddrOfInterest;
            DBG_vPrintf(TRACE_APP_OTA,"\n\nNwk Address oF server = %04x\n", sNodeState.u16NwkAddrOfServer);

            DBG_vPrintf(TRACE_APP_OTA|OTA_LNT,"OTA Server EP# = %d\n", sNodeState.u8OTAserverEP);
...
PUBLIC void vHandleIeeeAddressRsp( ZPS_tsAfEvent  * psStackEvent)
{
    ...
    if( sNodeState.u16NwkAddrOfServer == sIeeeAddrRsp.u16NwkAddrRemoteDev)
    {
        /*Make an entry in the OTA server tables*/
        sNodeState.u64IeeeAddrOfServer = sIeeeAddrRsp.u64IeeeAddrRemoteDev;
        DBG_vPrintf(TRACE_APP_OTA|1,"Entry Added NWK Addr 0x%04x IEEE Addr 0x%016llx",
                sNodeState.u16NwkAddrOfServer, sNodeState.u64IeeeAddrOfServer);
        ZPS_eAplZdoAddAddrMapEntry( sNodeState.u16NwkAddrOfServer,
                sNodeState.u64IeeeAddrOfServer,
                                    FALSE);
        sNodeState.bValid = TRUE;
        PDM_eSaveRecordData(PDM_ID_APP_ROUTER,&sNodeState,sizeof(tsNodeState));
        eOTA_SetServerAddress( app_u8GetDeviceEndpoint(), sNodeState.u64IeeeAddrOfServer, sNodeState.u16NwkAddrOfServer
                        );
...
PRIVATE void vGetIEEEAddress( void)
{
	...
	else
	{
		...
		uDstAddr.u16Addr = sNodeState.u16NwkAddrOfServer;
		bExtAddr = FALSE;
		sZdpIeeeAddrReq.u16NwkAddrOfInterest = sNodeState.u16NwkAddrOfServer;
...
PRIVATE bool_t bMatchRecieved(void)
{
    ...
    return (sNodeState.bValid == TRUE)? TRUE: FALSE;
}
/****************************************************************************
 *
...
{
    sOTA_PersistedData = sGetOTACallBackPersistdata();
    FLib_MemSet(&sOTA_PersistedData, 0, sizeof(tsOTA_PersistedData));
    PDM_eSaveRecordData(PDM_ID_OTA_DATA, &sOTA_PersistedData, sizeof(tsOTA_PersistedData));
}
